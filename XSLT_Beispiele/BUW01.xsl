<xsl:stylesheet xmlns:oai="http://www.openarchives.org/OAI/2.0/" xmlns:mets="http://www.loc.gov/METS/" xmlns:mods="http://www.loc.gov/mods/v3" xmlns:mix="http://www.loc.gov/mix/v20" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:dv="http://dfg-viewer.de/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:vl="http://visuallibrary.net/vl" xmlns:vls="http://semantics.de/vls" xmlns:marcxml="http://www.loc.gov/MARC21/slim" xmlns:epicur="urn:nbn:de:1111-2004033116" xmlns:zvdd="http://zvdd.gdz-cms.de/" xmlns:vlz="http://visuallibrary.net/vlz/1.0" xmlns:xlink="http://www.w3.org/1999/xlink" exclude-result-prefixes="xsl" version="1.0">
	...
	<xsl:template match="mets:dmdSec[@ID=$dmd_id]/mets:mdWrap[@MDTYPE=&apos;MODS&apos;]/mets:xmlData/mods:mods" mode="mods2dc">
				...
				<xsl:for-each select="./mods:titleInfo">
					<xsl:choose>
						...
						<xsl:otherwise>
							...
							<!-- Check if mods:number exist. If yes, connect with title -->
							<xsl:choose>
								<xsl:when test="./mods:subTitle and not(../mods:part[@type=&apos;host&apos;]/mods:detail/mods:number)">
									<dc:title>
										<xsl:value-of select="concat(./mods:title,&apos; : &apos;,./mods:subTitle)"/>
									</dc:title>
								</xsl:when>
								<xsl:when test="./mods:subTitle and ../mods:part[@type=&apos;host&apos;]/mods:detail/mods:number ">
									<dc:title>
										<xsl:value-of select="concat(./mods:title,&apos; : &apos;,./mods:subTitle, &apos; (&apos;, ../mods:part[@type=&apos;host&apos;]/mods:detail/mods:number, &apos;)&apos; )"/>
									</dc:title>
								</xsl:when>
								<xsl:when test="../mods:part[@type=&apos;host&apos;]/mods:detail/mods:number">
									<dc:title>
										<xsl:value-of select="concat(./mods:title, &apos; (&apos;, ../mods:part[@type=&apos;host&apos;]/mods:detail/mods:number, &apos;)&apos; )"/>
									</dc:title>
								</xsl:when>
								<xsl:otherwise>
									<dc:title>
		...
		<!-- Date -->
		<!-- Get mods:dateIssued value from element with attribute -->
		...
		<xsl:for-each select="./mods:originInfo/mods:dateIssued[not(@*)]">
			<dcterms:issued>
				<xsl:value-of select="."/>
			</dcterms:issued>
		</xsl:for-each>
		...
		<xsl:for-each select="./mods:originInfo/mods:publisher">
			<xsl:choose>
				<xsl:when test="../mods:place/mods:placeTerm">
					<dc:publisher>
						<xsl:value-of select="concat(../mods:place/mods:placeTerm, &apos;, &apos;, .)"/>
					</dc:publisher>
				</xsl:when>
				<xsl:otherwise>
					<dc:publisher>
		...
		<!-- Subject -->
		<xsl:for-each select="./mods:subject/child::*[not(self::mods:hierarchicalGeographic/mods:city)]">
		...
		<xsl:for-each select="./mods:identifier[not(@type=&apos;hbz-idn&apos;)]">
			<xsl:choose>
				...
				<xsl:when test="contains(./@type, &apos;vd&apos;)">
					<dc:identifier>
						<xsl:value-of select="concat(./@type,&apos;: &apos;,.)"/>
					</dc:identifier>
				</xsl:when>
		...
		<xsl:for-each select="./mods:relatedItem[@type=&apos;series&apos;]/mods:titleInfo[not(@type=&quot;alternative&quot;)]">
			<xsl:choose>
				<xsl:when test="./mods:subTitle">
					<xsl:choose>
						<xsl:when test="../mods:part[@order]/mods:detail[@type=&apos;volume&apos;]/mods:number">
							<dcterms:isPartOf>
								<xsl:value-of select="concat(./mods:title, &apos; : &apos;, ./mods:subTitle, &apos; (&apos;, ../mods:part[@order]/mods:detail[@type=&apos;volume&apos;]/mods:number, &apos;)&apos;)"/>
							</dcterms:isPartOf>
						</xsl:when>
						<xsl:otherwise>
							<dcterms:isPartOf>
				...
				<xsl:otherwise>
					<xsl:choose>
						<xsl:when test="../mods:part[@order]/mods:detail[@type=&apos;volume&apos;]/mods:number">
							<dcterms:isPartOf>
								<xsl:value-of select="concat(./mods:title, &apos; (&apos;, ../mods:part[@order]/mods:detail[@type=&apos;volume&apos;]/mods:number, &apos;)&apos;)"/>
							</dcterms:isPartOf>
						</xsl:when>
						<xsl:otherwise>
							<dcterms:isPartOf>
